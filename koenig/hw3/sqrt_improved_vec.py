...
precompute_to = 1500
precomputed_squares = np.square(np.array(range(precompute_to)))
...
def isperfect(n: int ):
    ...
    if n in precomputed_squares:
        return True, int(np.where(precomputed_squares == n)[0])
    elif n >= precompute_to**2:
        a = np.square(np.array(range(precompute_to, n // precompute_to + 1)))
        pos = np.where(a==n)[0]
        if pos.size != 0:  # replace None by the appropriate code.
            return True, int(pos)
    return False, n
    ### END CODE #####
...
def getLowUpper(n: int):
    ...
    """
    ### BEGIN CODE ####
    first_index_greater = np.nonzero( precomputed_squares > n)[0][0]
...
def main() :
    ...
    for n in first_test :
        myvalue = mysqrt(n)
        npvalue = np.sqrt(n)
        if round(myvalue, 2) == np.round(np.sqrt(n), 2):
