...
def isperfect(n: int ):
    ...
    i = 1
    while i * i <= n:  # by while loop just O(sqrt(n)) < O(n)
            if i * i == n:
                return True, i
            i += 1
    return False, n
    ...
    """
    # with vectorization
    i = np.arange(1, n+1 )  # array from 1 to 0.1*n
    squares = i * i  # Compute the squares of the array

    # vectorization: broadcasting with numpy - O(0.1*n) and better runtime
    x = squares <= n
    if np.any(squares[x] == n):
        perfect = np.argmax(squares == n)
        return True, i[perfect]
    else:
        return False, n
    """
...
def mysqrt(n: int, error_threshold=0.000000001) -> float:
     ...
     while maxsqrt-minsqrt >= error_threshold: # O(sqrt(n)log(n))
             """
