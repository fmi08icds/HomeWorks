...
def mutate(pop, strat) :
    ...
    normal_eta_matrix = np.broadcast_to(normal, (N,n)) ## COMMENTS should not be broadcast, please check the algo description.
    ...
    exp_term = (tau_prime * normal_eta_vec) + (tau * normal_eta_matrix) ##COMENTS: GOOD
...
def select(pop, new_pop, strat, new_strat, N, f, q=10) :
    ...
    for i in range(2*N):
        opponents = rng3.choice(all_pop, size=q, replace = False) # Good
...
def main() :
    ...
    ##COMMENTS: What about the Rosenbrock function?
    params = {
        'n' : args.length,
        'T' : args.T ,
        'q' : args.q,
    ...
    plt.title("Optimization of the function "+f.__doc__) ## COMMENTS: bug here.
