...
def get_low_and_upper_perfect_sqrt(n: int) -> Tuple[float, float]:
    ...
    i = 1                               # 1
    low = has_perfect_sqrt(n - i)       # n-i
    upper = has_perfect_sqrt(n + i)     # n+i

    while not low[0]:                   # l
        i += 1                          # l
        low = has_perfect_sqrt(n - i)   # l*n-i

    i = 1                               # 1
    while not upper[0]:                 # u
        i += 1                          # u
        upper = has_perfect_sqrt(n + i) # u*n+i

    minsqrt, maxsqrt = low[1], upper[1] # 1

    return minsqrt, maxsqrt             # 1

    # Worst case complexity 4 + n-i + n+i + 2*l + 2*u + l*n-i + u*n+i ~= n^2
...
def approximate_sqrt(n: int, error_threshold=0.000000001) -> float:
    ...
    checkup = has_perfect_sqrt(n)   # n
    if checkup[0]:                  # 1
        return checkup[1]           # 1

    iteration = 0                   # 1

    minsqrt, maxsqrt = get_low_and_upper_perfect_sqrt(n) # n^2

    rst = (minsqrt + maxsqrt) / 2   # 1

    while abs(rst**2 - n) >= error_threshold:  # n
        if rst**2 < n:              # n
            minsqrt = rst           # n
        else:                       # n
            maxsqrt = rst           # n

        rst = (minsqrt + maxsqrt) / 2      # n
        iteration += 1              # n
